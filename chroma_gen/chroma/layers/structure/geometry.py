...
class VirtualAtomsCA(nn.Module):
    ...
    def forward(self, X: torch.Tensor, C: torch.LongTensor) -> torch.Tensor:
        ...
        mask = (C > 0).type(torch.float64).unsqueeze(-1)
...
class InternalCoords(nn.Module):
    ...
    def forward(
        ...
        mask_L = mask_L.type(torch.float64)
        mask_A = mask_A.type(torch.float64)
        mask_D = mask_D.type(torch.float64)

        # Masks for branched oxygen
        mask_O_D = torch.eq(C[:, :-1], C[:, 1:])
        mask_O_D = mask_O_D.type(torch.float64)
        mask_O_A = mask
        mask_O_L = mask
