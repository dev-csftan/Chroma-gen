...
class RigidTransform(nn.Module):
    ...
    def __init__(
        ...
        q_init = torch.tensor(q_init, dtype=torch.float64) / self.scale_q
...
class BackboneBuilder(nn.Module):
    ...
    def forward(
        ...
        u_i_minus_2 = torch.tensor([[1.0, 0, 0]] * N_batch, dtype=torch.float64).to(
            device
        )
        u_i_minus_1 = torch.tensor([[0, 1.0, 0]] * N_batch, dtype=torch.float64).to(
            device
        )
...
class FrameBuilder(nn.Module):
    ...
    def _build_O(self, X_chain: torch.Tensor, C: torch.LongTensor):
        ...
        ones = torch.ones(list(C.shape), dtype=torch.float64, device=C.device)
...
def expand_chain_map(C: torch.LongTensor) -> torch.Tensor:
    ...
    mask_C = (idx_expand == C_expand).type(torch.float64)
    return mask_C
