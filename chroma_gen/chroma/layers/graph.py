    ...
    def forward(
        ...
        for i, layer in enumerate(self.layers):
            ...
            if mask_i is not None:
                node_h = node_h * (mask_i.unsqueeze(-1) != 0).type(torch.float64)
            if mask_ij is not None:
                edge_h = edge_h * (mask_ij.unsqueeze(-1) != 0).type(torch.float64)
...
class GraphLayer(nn.Module):
    ...
    def forward(
        ...
        if mask_i is not None:
            mask_i = (mask_i != 0).type(torch.float64)
        if mask_ij is not None:
            mask_ij = (mask_ij != 0).type(torch.float64)
...
def transpose_edge_idx(
    ...
    edge_idx_match = (edge_idx_neighbors == residue_i).type(torch.float64)
    ...
    def forward(
        ...
        if (mask is None) or (self.norm_type == "transformer"):
            ...
            for mask_id in mask.unique():
                ...
                tmask = (mask == mask_id).type(torch.float64)
